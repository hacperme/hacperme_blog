...
想要使用 Trace 32 Simulators 离线解析 ramdump，必须具备以下 3 个条件：
- 知道当前寄存器的值
- 能够获取当前运行状态下 ram 的数据
- 能否获取到当前使用的 elf 文件  
...
新增一个 bk_dump_parser.py 脚本，从log中提取ram、tcm、寄存器的文件，并写入到文件。
```diff
...
新增 trace32 脚本，用于加载和解析 ramdump文件。
...
将 dumplog elf文件放到 bkdump 目录, 执行脚本 bk_dump_parser.py 解析 dumplog。
