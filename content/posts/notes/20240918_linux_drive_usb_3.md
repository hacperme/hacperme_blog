...
枚举完成之前，USB设备要经过一系列的状态变化，才能最终完成枚举，。
这些状态是
- 连接状态（attached）
  如果USB设备连接到主机，就处于连接状态
- 供电状态（powered）
  VBUS有电后，进入供电状态
- 默认状态（default）
  USB设备进入供电状态后，在被复位之前，不能响应总线上的任何事务（Transaction）。只有当USB设备被复位，处于默认状态后，才会响应主机发送过来的请求。
- 地址状态（address）
  在USB设备被复位后，且在USB主机给USB设备设置一个新的地址之前，所有的USB设备使用默认的0地址与主机通信。主机发送的设置地址SetAddress（）请求后，USB 设备就会得到一个唯一的地址。USB设备会保存并使用该地址与主机通信，直到设备被复位或者断开。
- 配置状态（configured）
  在USB设备的功能能够使用之前，USB设备和USB主机必须协商确定功能相关的配置项，所有的配置项以描述符的形式提供。
  USB主机通过获取描述符GetDescriptor（）请求获得USB设备相关的描述符，通过设置接口 SetInterface（）和设置配置SetConfiguration（）来设置设备的相关配置项。一旦相关功能参数被配置后，USB设备就能正常工作了。
- 挂起状态（suspended）
...
枚举的第一步是获取设备描述符，主机通过GetDescriptor（Devicetype）请求来获取设备的设备描述符。对于高速设备和低速设备，用
于控制传输的端点0支持的最大包长度是确定的，分别是64字节和8字节。全速设备，端点0支持的最大包长度可能是8字节、16字节、32字节或者64字节。
